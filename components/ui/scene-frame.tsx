...
export function SceneFrame({ src, alt, className, aspect = "aspect-[4/5]" }: SceneFrameProps) {
  return (
    <div className={cn("group relative", className)}>
      ...
      <figure className="relative overflow-hidden rounded-[1.75rem] border border-[var(--color-line)] bg-[var(--color-ivory)] p-2 shadow-[var(--shadow-soft)]">
        <div className={cn("relative overflow-hidden rounded-[1.4rem]", aspect)}>
          {/* SVG scene: plain img so next/image SVG optimization is not required */}
          ...
          <span
            aria-hidden
            className="pointer-events-none absolute inset-0 rounded-[1.4rem] bg-[linear-gradient(180deg,transparent_55%,rgba(42,33,26,0.28))] mix-blend-multiply"
          />
        </div>
      </figure>
    </div>
